...
if __name__ == '__main__':
    ...
    test_dir = f"./test_out/parsoda_pycompss_dev/parsoda_pycompss_dev_{now_time}"
    ...
    for app in apps_list:
        for chunk_size in chunk_sizes:
            ...
            for test_index in range(0, test_num):
                print(f"Starting: app={app}; test-id={test_index}; cores=#dev-cores; chunk-size={chunk_size} MB")
            
                app_logs_dir = f"{test_logs_dir}/{app}/dev-cores.test{test_index}"
                if not os.path.exists(app_logs_dir):
                    os.makedirs(app_logs_dir)
                    
                    
                cmd = (
                    f"PYTHONPATH=. runcompss --python_interpreter=python3 "
                    f"--resources=./config/dev-resources.xml "
                    f"--project=./config/dev-project.xml "
                    f"--jvm_workers_opts=\"-Xmx3900M\" "
                    f"parsoda/apps/{app}.py --chunk-size {chunk_size} "
                    f" > {app_logs_dir}/{app}.dev-cores.chunk{chunk_size}.test{test_index}.log"
                )
                print(f"command: {cmd}")
                    ...
                    try:
                        with open("parsoda_report.csv", "r") as f:
                            ...
                            with open(test_results_file, "a") as f:
                                f.write(f"{app};#dev-cores;{num_partitions_given};{chunk_size_given};{crawling_time};{filter_time};{map_time};{split_time};{reduce_time};{analysis_time};{visualization_time};{filter_to_reduce_time};{total_execution_time};{total_time}\n".replace(".", ","))
